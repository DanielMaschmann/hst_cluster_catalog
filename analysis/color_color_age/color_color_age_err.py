...
for catalog_version in ['SEDfix_Ha1_inclusiveGCcc_inclusiveGCclass',
    ...
    age_std_map_hum_1 = np.zeros((len(x_bins), len(y_bins))) * np.nan
    age_std_map_hum_2 = np.zeros((len(x_bins), len(y_bins))) * np.nan
    age_std_map_hum_3 = np.zeros((len(x_bins), len(y_bins))) * np.nan
    age_std_map_ml_1 = np.zeros((len(x_bins), len(y_bins))) * np.nan
    age_std_map_ml_2 = np.zeros((len(x_bins), len(y_bins))) * np.nan
    age_std_map_ml_3 = np.zeros((len(x_bins), len(y_bins))) * np.nan
    ...
    for x_index in range(len(x_bins)-1):
        for y_index in range(len(y_bins)-1):
            ...
            if sum(mask_selected_obj_hum_1) > threshold_hum:
                age_std_map_hum_1[x_index, y_index] = np.nanstd(np.log10(age_hum[mask_selected_obj_hum_1]))
            if sum(mask_selected_obj_hum_2) > threshold_hum:
                age_std_map_hum_2[x_index, y_index] = np.nanstd(np.log10(age_hum[mask_selected_obj_hum_2]))
            if sum(mask_selected_obj_hum_3) > threshold_hum:
                age_std_map_hum_3[x_index, y_index] = np.nanstd(np.log10(age_hum[mask_selected_obj_hum_3]))

            if sum(mask_selected_obj_ml_1) > threshold_ml:
                age_std_map_ml_1[x_index, y_index] = np.nanstd(np.log10(age_ml[mask_selected_obj_ml_1]))
            if sum(mask_selected_obj_ml_2) > threshold_ml:
                age_std_map_ml_2[x_index, y_index] = np.nanstd(np.log10(age_ml[mask_selected_obj_ml_2]))
            if sum(mask_selected_obj_ml_3) > threshold_ml:
                age_std_map_ml_3[x_index, y_index] = np.nanstd(np.log10(age_ml[mask_selected_obj_ml_3]))


    figure = plt.figure(figsize=(29, 20))
    fontsize = 28

    cmap_age_std = matplotlib.cm.get_cmap('rainbow')
    norm_age_std = matplotlib.colors.Normalize(vmin=0, vmax=2)



    ax_age_std_hum_1 = figure.add_axes([0.04, 0.52, 0.32, 0.46])
    ax_age_std_ml_1 = figure.add_axes([0.04, 0.05, 0.32, 0.46])

    ax_age_std_hum_2 = figure.add_axes([0.33, 0.52, 0.32, 0.46])
    ax_age_std_ml_2 = figure.add_axes([0.33, 0.05, 0.32, 0.46])

    ax_age_std_hum_3 = figure.add_axes([0.62, 0.52, 0.32, 0.46])
    ax_age_std_ml_3 = figure.add_axes([0.62, 0.05, 0.32, 0.46])

    ax_cbar_age_std = figure.add_axes([0.94, 0.2, 0.015, 0.6])


    ax_age_std_hum_1.imshow(age_std_map_hum_1.T, origin='lower', extent=(x_lim_ubvi[0], x_lim_ubvi[1], y_lim_ubvi[1], y_lim_ubvi[0]), cmap=cmap_age_std, norm=norm_age_std)
    ax_age_std_hum_2.imshow(age_std_map_hum_2.T, origin='lower', extent=(x_lim_ubvi[0], x_lim_ubvi[1], y_lim_ubvi[1], y_lim_ubvi[0]), cmap=cmap_age_std, norm=norm_age_std)
    ax_age_std_hum_3.imshow(age_std_map_hum_3.T, origin='lower', extent=(x_lim_ubvi[0], x_lim_ubvi[1], y_lim_ubvi[1], y_lim_ubvi[0]), cmap=cmap_age_std, norm=norm_age_std)

    ax_age_std_ml_1.imshow(age_std_map_ml_1.T, origin='lower', extent=(x_lim_ubvi[0], x_lim_ubvi[1], y_lim_ubvi[1], y_lim_ubvi[0]), cmap=cmap_age_std, norm=norm_age_std)
    ax_age_std_ml_2.imshow(age_std_map_ml_2.T, origin='lower', extent=(x_lim_ubvi[0], x_lim_ubvi[1], y_lim_ubvi[1], y_lim_ubvi[0]), cmap=cmap_age_std, norm=norm_age_std)
    ax_age_std_ml_3.imshow(age_std_map_ml_3.T, origin='lower', extent=(x_lim_ubvi[0], x_lim_ubvi[1], y_lim_ubvi[1], y_lim_ubvi[0]), cmap=cmap_age_std, norm=norm_age_std)




    ax_age_std_hum_1.plot(model_vi_sol, model_ub_sol, color='darkred', linewidth=4, linestyle='-', label=r'BC03, Z$_{\odot}$')
    ax_age_std_hum_1.plot(model_vi_sol50, model_ub_sol50, color='darkorange', linewidth=4, linestyle='--', label=r'BC03, Z$_{\odot}/50$')
    ax_age_std_hum_2.plot(model_vi_sol, model_ub_sol, color='darkred', linewidth=4, linestyle='-')
    ax_age_std_hum_2.plot(model_vi_sol50, model_ub_sol50, color='darkorange', linewidth=4, linestyle='--')

    ax_age_std_hum_3.plot(model_vi_sol, model_ub_sol, color='darkred', linewidth=4, linestyle='-')
    ax_age_std_hum_3.plot(model_vi_sol50, model_ub_sol50, color='darkorange', linewidth=4, linestyle='--')
    ax_age_std_ml_1.plot(model_vi_sol, model_ub_sol, color='darkred', linewidth=4, linestyle='-')
    ax_age_std_ml_1.plot(model_vi_sol50, model_ub_sol50, color='darkorange', linewidth=4, linestyle='--')

    ax_age_std_ml_2.plot(model_vi_sol, model_ub_sol, color='darkred', linewidth=4, linestyle='-')
    ax_age_std_ml_2.plot(model_vi_sol50, model_ub_sol50, color='darkorange', linewidth=4, linestyle='--')
    ax_age_std_ml_3.plot(model_vi_sol, model_ub_sol, color='darkred', linewidth=4, linestyle='-')
    ax_age_std_ml_3.plot(model_vi_sol50, model_ub_sol50, color='darkorange', linewidth=4, linestyle='--')
    ...
    helper_func.plot_reddening_vect(ax=ax_age_std_hum_1, x_color_1='v', x_color_2='i',  y_color_1='u', y_color_2='b',
    ...
    helper_func.plot_reddening_vect(ax=ax_age_std_hum_2, x_color_1='v', x_color_2='i',  y_color_1='u', y_color_2='b',
                           x_color_int=vi_int, y_color_int=ub_int, av_val=1,
                           linewidth=4, line_color='k', text=False, fontsize=fontsize)
    helper_func.plot_reddening_vect(ax=ax_age_std_hum_3, x_color_1='v', x_color_2='i',  y_color_1='u', y_color_2='b',
                           x_color_int=vi_int, y_color_int=ub_int, av_val=1,
                           linewidth=4, line_color='k', text=False, fontsize=fontsize)

    helper_func.plot_reddening_vect(ax=ax_age_std_ml_1, x_color_1='v', x_color_2='i',  y_color_1='u', y_color_2='b',
                           x_color_int=vi_int, y_color_int=ub_int, av_val=1,
                           linewidth=4, line_color='k', text=False, fontsize=fontsize-4, x_text_offset=-0.1, y_text_offset=-0.3)
    helper_func.plot_reddening_vect(ax=ax_age_std_ml_2, x_color_1='v', x_color_2='i',  y_color_1='u', y_color_2='b',
                           x_color_int=vi_int, y_color_int=ub_int, av_val=1,
                           linewidth=4, line_color='k', text=False, fontsize=fontsize)
    helper_func.plot_reddening_vect(ax=ax_age_std_ml_3, x_color_1='v', x_color_2='i',  y_color_1='u', y_color_2='b',
                           x_color_int=vi_int, y_color_int=ub_int, av_val=1,
                           linewidth=4, line_color='k', text=False, fontsize=fontsize)


    ColorbarBase(ax_cbar_age_std, orientation='vertical', cmap=cmap_age_std, norm=norm_age_std, extend='neither', ticks=None)
    ax_cbar_age_std.set_ylabel(r'log($\sigma_{rm Age}$)', labelpad=-4, fontsize=fontsize + 5)
    ax_cbar_age_std.tick_params(axis='both', which='both', width=2, direction='in', right=True, labelright=True, labelsize=fontsize)


    ax_age_std_hum_1.text(x_lim_ubvi[0] + (x_lim_ubvi[1]-x_lim_ubvi[0])*0.95, y_lim_ubvi[0] + (y_lim_ubvi[1]-y_lim_ubvi[0])*0.93,
                     'Class 1 (Hum)', horizontalalignment='right', verticalalignment='center', fontsize=fontsize)
    ax_age_std_ml_1.text(x_lim_ubvi[0] + (x_lim_ubvi[1]-x_lim_ubvi[0])*0.95, y_lim_ubvi[0] + (y_lim_ubvi[1]-y_lim_ubvi[0])*0.93,
                     'Class 1 (ML)', horizontalalignment='right', verticalalignment='center', fontsize=fontsize)
    ax_age_std_hum_2.text(x_lim_ubvi[0] + (x_lim_ubvi[1]-x_lim_ubvi[0])*0.95, y_lim_ubvi[0] + (y_lim_ubvi[1]-y_lim_ubvi[0])*0.93,
                     'Class 2 (Hum)', horizontalalignment='right', verticalalignment='center', fontsize=fontsize)
    ax_age_std_ml_2.text(x_lim_ubvi[0] + (x_lim_ubvi[1]-x_lim_ubvi[0])*0.95, y_lim_ubvi[0] + (y_lim_ubvi[1]-y_lim_ubvi[0])*0.93,
                     'Class 2 (ML)', horizontalalignment='right', verticalalignment='center', fontsize=fontsize)
    ax_age_std_hum_3.text(x_lim_ubvi[0] + (x_lim_ubvi[1]-x_lim_ubvi[0])*0.95, y_lim_ubvi[0] + (y_lim_ubvi[1]-y_lim_ubvi[0])*0.93,
                     'Compact associations (Hum)', horizontalalignment='right', verticalalignment='center', fontsize=fontsize)
    ax_age_std_ml_3.text(x_lim_ubvi[0] + (x_lim_ubvi[1]-x_lim_ubvi[0])*0.95, y_lim_ubvi[0] + (y_lim_ubvi[1]-y_lim_ubvi[0])*0.93,
                     'Compact associations (ML)', horizontalalignment='right', verticalalignment='center', fontsize=fontsize)

    ax_age_std_hum_1.set_xlim(x_lim_ubvi)
    ax_age_std_hum_2.set_xlim(x_lim_ubvi)
    ax_age_std_hum_3.set_xlim(x_lim_ubvi)
    ax_age_std_ml_1.set_xlim(x_lim_ubvi)
    ax_age_std_ml_2.set_xlim(x_lim_ubvi)
    ax_age_std_ml_3.set_xlim(x_lim_ubvi)

    ax_age_std_hum_1.set_ylim(y_lim_ubvi)
    ax_age_std_hum_2.set_ylim(y_lim_ubvi)
    ax_age_std_hum_3.set_ylim(y_lim_ubvi)
    ax_age_std_ml_1.set_ylim(y_lim_ubvi)
    ax_age_std_ml_2.set_ylim(y_lim_ubvi)
    ax_age_std_ml_3.set_ylim(y_lim_ubvi)

    ax_age_std_hum_2.set_yticklabels([])
    ax_age_std_hum_3.set_yticklabels([])
    ax_age_std_ml_2.set_yticklabels([])
    ax_age_std_ml_3.set_yticklabels([])

    ax_age_std_hum_1.set_xticklabels([])
    ax_age_std_hum_2.set_xticklabels([])
    ax_age_std_hum_3.set_xticklabels([])


    ax_age_std_hum_1.set_ylabel('U (F336W) - B (F438W/F435W'+'$^*$'+')', fontsize=fontsize)
    ax_age_std_ml_1.set_ylabel('U (F336W) - B (F438W/F435W'+'$^*$'+')', fontsize=fontsize)

    ax_age_std_ml_1.set_xlabel('V (F555W) - I (F814W)', fontsize=fontsize)
    ax_age_std_ml_2.set_xlabel('V (F555W) - I (F814W)', fontsize=fontsize)
    ax_age_std_ml_3.set_xlabel('V (F555W) - I (F814W)', fontsize=fontsize)

    ax_age_std_hum_1.legend(frameon=False, loc=3, fontsize=fontsize)

    ax_age_std_hum_1.tick_params(axis='both', which='both', width=1.5, length=4, right=True, top=True, direction='in', labelsize=fontsize)
    ax_age_std_hum_2.tick_params(axis='both', which='both', width=1.5, length=4, right=True, top=True, direction='in', labelsize=fontsize)
    ax_age_std_hum_3.tick_params(axis='both', which='both', width=1.5, length=4, right=True, top=True, direction='in', labelsize=fontsize)
    ax_age_std_ml_1.tick_params(axis='both', which='both', width=1.5, length=4, right=True, top=True, direction='in', labelsize=fontsize)
    ax_age_std_ml_2.tick_params(axis='both', which='both', width=1.5, length=4, right=True, top=True, direction='in', labelsize=fontsize)
    ax_age_std_ml_3.tick_params(axis='both', which='both', width=1.5, length=4, right=True, top=True, direction='in', labelsize=fontsize)

    plt.tight_layout()
    plt.savefig('plot_output/color_color_age_std_%s.png' % catalog_version)
    plt.savefig('plot_output/color_color_age_std_%s.pdf' % catalog_version)
    plt.clf()
    plt.cla()
