...
for index in range(0, 20):
    ...
    ax[row_index, col_index].hist(v_mag_hum, bins=abs_v_mag_bins, density=True, histtype='step', color='red', linewidth=1.2, label='Human')
    ax[row_index, col_index].hist(v_mag_ml, bins=abs_v_mag_bins, density=True, histtype='step', color='k', linewidth=1.2, label='ML')
    ...
    if median_v_mag_ml > faintest_v_band_mag_hum:
        ...
        description_str = target.upper()+'\nd='+str(dist)+' Mpc'
    anchored_left = AnchoredText(description_str, loc='upper left', borderpad=0.1, frameon=False, prop=dict(size=fontsize-4))
    ...
    if index == 0:
        ax[row_index, col_index].legend(frameon=False, fontsize=fontsize-4)
...
fig.text(0.5, 0.08, 'V [mag]', ha='center', fontsize=fontsize)
fig.text(0.08, 0.5, 'Frequency', va='center', rotation='vertical', fontsize=fontsize)

fig.subplots_adjust(wspace=0, hspace=0)
fig.savefig('plot_output/v_mag_1.png', bbox_inches='tight', dpi=300)
fig.savefig('plot_output/v_mag_1.pdf', bbox_inches='tight', dpi=300)
...
fig.subplots_adjust(wspace=0, hspace=0)
fig.savefig('plot_output/v_mag_2.png', bbox_inches='tight', dpi=300)
fig.savefig('plot_output/v_mag_2.pdf', bbox_inches='tight', dpi=300)
