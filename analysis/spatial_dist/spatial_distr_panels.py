...
def get_alma_maps(target, new_wcs, new_shape):
    ...
    return alma_data_reprojected, alma_borders_reprojected


def get_convolved_hist(shape, x_data, y_data, color, kernel_size_prop=0.01, gamma=4.0, gammacorr_color=2.8):
    x_bins = np.linspace(0, shape[0], shape[0])
    y_bins = np.linspace(0, shape[1], shape[1])

    # create histogram
    hist = np.histogram2d(x_data, y_data, bins=(x_bins, y_bins))[0].T

    # convolve with a square kernel
    kernel_size = int(hist.shape[0] * kernel_size_prop) * 6
    if (kernel_size % 2) == 0:
        kernel_size += 1
    kernel_fwhm = hist.shape[0] * kernel_size_prop
    kernel = make_2dgaussian_kernel(kernel_fwhm, size=kernel_size)
    hist = convolve(hist, kernel)

    grey_scale_img = mcf.greyRGBize_image(hist, rescalefn='asinh', scaletype='abs', min_max=[0.0, 1/kernel_fwhm],
                                          gamma=gamma, checkscale=False)
    colorized_img = mcf.colorize_image(grey_scale_img, color, colorintype='hex',
                                                 gammacorr_color=gammacorr_color)

    rgb_img = mcf.combine_multicolor([colorized_img], gamma=2.8, inverse=False)
    mask_0 = hist < (1/kernel_fwhm / 100)
    rgb_img[mask_0] = 1.0

    return rgb_img


def get_hst_rgb(galaxy_name, new_wcs, shape):
    ...
    rgb_img_hst_data_r = plotting_tools.reproject_image(data=hdu_rgb_img_hst[0].data[0],
                                                            ...
                                                            new_wcs=new_wcs, new_shape=shape)
...
def get_histograms(galaxy_name, kernel_size_prop=0.01):
    ...
    ra_target_ml = ra_ml[target_mask]
    dec_target_ml = dec_ml[target_mask]

    # get min and max boundaries
    left_ra = np.nanmax(ra_target_ml)
    right_ra = np.nanmin(ra_target_ml)
    lower_dec = np.nanmin(dec_target_ml)
    upper_dec = np.nanmax(dec_target_ml)

    # get WCS
    wcs_hist = hf.construct_wcs(ra_min=left_ra + add_side_space_deg, ra_max=right_ra - add_side_space_deg,
                                dec_min=lower_dec - add_side_space_deg, dec_max=upper_dec + add_side_space_deg,
                                img_shape=n_bins, quadratic_image=True)

    # convert coordinates to pixel numbers
    pixel_coords_ml = wcs_hist.world_to_pixel(SkyCoord(ra=ra_target_ml*u.deg, dec=dec_target_ml*u.deg))

    young_ml = age_ml < 10

    mask_ogc_ml = (in_hull_gc_ml * detectable_ml * np.invert(young_ml))[target_mask]
    mask_map_ml = (in_hull_cascade_ml * detectable_ml * np.invert(young_ml))[target_mask]
    mask_ycl_ml = ((detectable_ml * young_ml * (in_hull_gc_ml + in_hull_cascade_ml)) +
                   (in_hull_young_ml * detectable_ml * np.invert(in_hull_cascade_ml)))[target_mask]

    hist_ogc = get_convolved_hist(shape=n_bins, x_data=pixel_coords_ml[0][mask_ogc_ml], y_data=pixel_coords_ml[1][mask_ogc_ml],
                                 color=red_color, kernel_size_prop=kernel_size_prop, gamma=4.0, gammacorr_color=2.8)
    hist_map = get_convolved_hist(shape=n_bins, x_data=pixel_coords_ml[0][mask_map_ml], y_data=pixel_coords_ml[1][mask_map_ml],
                                 color=green_color, kernel_size_prop=kernel_size_prop, gamma=4.0, gammacorr_color=2.8)
    hist_ycl = get_convolved_hist(shape=n_bins, x_data=pixel_coords_ml[0][mask_ycl_ml], y_data=pixel_coords_ml[1][mask_ycl_ml],
                                 color=blue_color, kernel_size_prop=kernel_size_prop, gamma=4.0, gammacorr_color=2.8)

    return hist_ogc, hist_map, hist_ycl, wcs_hist
...
def plot_panels(ax_img_r, ax_img_g, ax_img_b, ax_img_hst, img_r, img_g, img_b, img_hst,
    ...
    if plot_contours:
        alma_kernel_fwhm = 5
        alma_kernel_size = 15
        alma_kernel = make_2dgaussian_kernel(alma_kernel_fwhm, size=alma_kernel_size)
        contour_data = convolve(contour_data, alma_kernel)

        # alma_levels = [np.nanpercentile(contour_data, 95), np.nanpercentile(contour_data, 97),
        #           np.nanpercentile(contour_data, 99)]
        levels = [np.nanpercentile(contour_data, 95)]
...
for index in range(10):
    ...
    ogc_img_1, map_img_1, ycl_img_1, wcs_hist_1 = get_histograms(galaxy_name=target_1, kernel_size_prop=kernel_size_prop)
    rgb_img_hst_img_1 = get_hst_rgb(galaxy_name=target_1, new_wcs=wcs_hist_1, shape=n_bins)
    alma_data_1, alma_borders_1 = get_alma_maps(target=target_1, new_wcs=wcs_hist_1, new_shape=n_bins)

    ogc_img_2, map_img_2, ycl_img_2, wcs_hist_2 = get_histograms(galaxy_name=target_2, kernel_size_prop=kernel_size_prop)
    rgb_img_hst_img_2 = get_hst_rgb(galaxy_name=target_2, new_wcs=wcs_hist_2, shape=n_bins)
    alma_data_2, alma_borders_2 = get_alma_maps(target=target_2, new_wcs=wcs_hist_2, new_shape=n_bins)

    if index != 9:
        ogc_img_3, map_img_3, ycl_img_3, wcs_hist_3 = get_histograms(galaxy_name=target_3, kernel_size_prop=kernel_size_prop)
        rgb_img_hst_img_3 = get_hst_rgb(galaxy_name=target_3, new_wcs=wcs_hist_3, shape=n_bins)
        alma_data_3, alma_borders_3 = get_alma_maps(target=target_3, new_wcs=wcs_hist_3, new_shape=n_bins)

        ogc_img_4, map_img_4, ycl_img_4, wcs_hist_4 = get_histograms(galaxy_name=target_4, kernel_size_prop=kernel_size_prop)
        rgb_img_hst_img_4 = get_hst_rgb(galaxy_name=target_4, new_wcs=wcs_hist_4, shape=n_bins)
        alma_data_4, alma_borders_4 = get_alma_maps(target=target_4, new_wcs=wcs_hist_4, new_shape=n_bins)
    ...
    ax_img_ogc_1 = figure.add_axes([ax_vertical1, ax_horizont1, ax_size, ax_size], projection=wcs_hist_1)
    ...
    ax_img_ogc_2 = figure.add_axes([ax_vertical2, ax_horizont1, ax_size, ax_size], projection=wcs_hist_2)
    ...
    if index != 9:
        ...
        ax_img_ogc_3 = figure.add_axes([ax_vertical3, ax_horizont1, ax_size, ax_size], projection=wcs_hist_3)
        ...
        ax_img_ogc_4 = figure.add_axes([ax_vertical4, ax_horizont1, ax_size, ax_size], projection=wcs_hist_4)
    ...
    plot_panels(ax_img_r=ax_img_ogc_1, ax_img_g=ax_img_map_1, ax_img_b=ax_img_ycl_1,
                ax_img_hst=ax_img_hst_1,
                img_r=ogc_img_1, img_g=map_img_1, img_b=ycl_img_1, img_hst=rgb_img_hst_img_1,
    ...
    plot_panels(ax_img_r=ax_img_ogc_2, ax_img_g=ax_img_map_2, ax_img_b=ax_img_ycl_2,
                ax_img_hst=ax_img_hst_2,
                img_r=ogc_img_2, img_g=map_img_2, img_b=ycl_img_2, img_hst=rgb_img_hst_img_2,
                contour_data=alma_data_2, contour_borders=alma_borders_2, contour_color='magenta', alpha=0.7, linewidths=4)
    if index != 9:
        plot_panels(ax_img_r=ax_img_ogc_3, ax_img_g=ax_img_map_3, ax_img_b=ax_img_ycl_3,
                    ax_img_hst=ax_img_hst_3,
                    img_r=ogc_img_3, img_g=map_img_3, img_b=ycl_img_3, img_hst=rgb_img_hst_img_3,
                    contour_data=alma_data_3, contour_borders=alma_borders_3, contour_color='magenta', alpha=0.7, linewidths=4)

        plot_panels(ax_img_r=ax_img_ogc_4, ax_img_g=ax_img_map_4, ax_img_b=ax_img_ycl_4,
                    ax_img_hst=ax_img_hst_4,
                    img_r=ogc_img_4, img_g=map_img_4, img_b=ycl_img_4, img_hst=rgb_img_hst_img_4,
                    contour_data=alma_data_4, contour_borders=alma_borders_4, contour_color='magenta', alpha=0.7, linewidths=4)

    plot_names(ax_img_r=ax_img_ogc_1, ax_img_g=ax_img_map_1, ax_img_b=ax_img_ycl_1,
               ax_img_hst=ax_img_hst_1, target=target_1, delta_ms=delta_ms_1, shape=ogc_img_1.shape, fontsize=fontsize, plot_identifier=True)
    plot_names(ax_img_r=ax_img_ogc_2, ax_img_g=ax_img_map_2, ax_img_b=ax_img_ycl_2,
           ax_img_hst=ax_img_hst_2, target=target_2, delta_ms=delta_ms_2, shape=ogc_img_2.shape, fontsize=fontsize, plot_identifier=False)
    if index != 9:
        plot_names(ax_img_r=ax_img_ogc_3, ax_img_g=ax_img_map_3, ax_img_b=ax_img_ycl_3,
               ax_img_hst=ax_img_hst_3, target=target_3, delta_ms=delta_ms_3, shape=ogc_img_3.shape, fontsize=fontsize, plot_identifier=False)
        plot_names(ax_img_r=ax_img_ogc_4, ax_img_g=ax_img_map_4, ax_img_b=ax_img_ycl_4,
                   ax_img_hst=ax_img_hst_4, target=target_4, delta_ms=delta_ms_4, shape=ogc_img_4.shape, fontsize=fontsize, plot_identifier=False)
    ...
    plotting_tools.arr_axis_params(ax=ax_img_ogc_1, ra_tick_label=False, dec_tick_label=True,
    ...
    plotting_tools.arr_axis_params(ax=ax_img_ogc_2, ra_tick_label=False, dec_tick_label=True,
    ...
    if index != 9:
        plotting_tools.arr_axis_params(ax=ax_img_ogc_3, ra_tick_label=False, dec_tick_label=True,
                                           ra_axis_label=' ', dec_axis_label=' ',
                                           ra_minpad=0.3, dec_minpad=0.5, tick_color='k',
        ...
        plotting_tools.arr_axis_params(ax=ax_img_ogc_4, ra_tick_label=False, dec_tick_label=True,
                                           ra_axis_label=' ', dec_axis_label=' ',
                                           ra_minpad=0.3, dec_minpad=0.5, tick_color='k',
                                           fontsize=fontsize, labelsize=fontsize, ra_tick_num=3, dec_tick_num=4)
