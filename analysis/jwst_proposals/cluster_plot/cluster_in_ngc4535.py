...
rgb_cutout = visualization_access.get_rgb_img(data_r=cutout_dict['F814W_img_cutout'].data,
                                              ...
                                              min_max_g=min_max,
                                              min_max_b=min_max,
                                              gamma_r=gamma, gamma_g=gamma, gamma_b=gamma,
                                              gamma_corr_r=gamma, gamma_corr_g=gamma, gamma_corr_b=gamma, combined_gamma=2.2)
...
fontsize = 40
ax_img = figure.add_axes([0.04, 0.035, 0.955, 0.935], projection=img_wcs_v)

ax_img.imshow(rgb_cutout)

if cluster_class == 'Human':
    ax_img.scatter(coords_in_cutout_pix_hum_cl12[0][mask_in_cutout_hum_cl12],
                   coords_in_cutout_pix_hum_cl12[1][mask_in_cutout_hum_cl12],
                   color='white', marker='o', s=280, linewidth=2, facecolor='None')
else:
    ax_img.scatter(coords_in_cutout_pix_ml_cl12[0][mask_in_cutout_ml_cl12],
...
ax_img.set_title(target_name + '  (' + cluster_class + ') Class 1+2' , fontsize=fontsize+10)

plt.savefig('plot_output/cluster_%s_%s.png' % (target, cluster_class))
# plt.show()
