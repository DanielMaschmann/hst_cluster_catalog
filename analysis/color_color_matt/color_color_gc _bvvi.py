...
mask_detect_bvvi_hum = detect_u_hum * detect_b_hum * detect_v_hum * detect_i_hum
...
density_with_points(ax=ax, x=color_vi_hum[mask_class_1_hum * mask_good_colors_bvvi_hum],
...
# plot model tracks
ax.plot(model_vi_sol, model_bv_sol, color='green', linewidth=4, linestyle='-', zorder=10)
ax.plot(model_vi_sol50, model_bv_sol50, color='gray', linewidth=4, linestyle='--', zorder=10)

ax.set_title('Class 1', fontsize=fontsize)

ax.set_xlabel('B (F438W/F435W'+'$^*$'+') - V (F555W)', fontsize=fontsize)

ax.set_ylabel('U (F336W) - B (F438W/F435W'+'$^*$'+')', fontsize=fontsize)

ax.set_xlim(x_lim_vi)

ax.set_ylim(y_lim_bv)

ax.tick_params(axis='both', which='both', width=1.5, length=4, right=True, top=True, direction='in', labelsize=fontsize)

plt.tight_layout()
fig.savefig('plot_output/color_color_c1_heat_map_bvvi.png')
fig.savefig('plot_output/color_color_c1_heat_map_bvvi.pdf')
fig.clf()
plt.cla()
...
# plot the density contours
contours(ax=ax, x=color_vi_hum[mask_class_1_hum * mask_good_colors_bvvi_hum],
         y=color_bv_hum[mask_class_1_hum * mask_good_colors_bvvi_hum])
...
plt.tight_layout()
fig.savefig('plot_output/color_color_c1_contour_bvvi.png')
fig.savefig('plot_output/color_color_c1_contour_bvvi.pdf')
fig.clf()
plt.cla()
