...
x_data = color_vi_hum[mask_good_colors_ubvi_hum * mask_class_2_hum]
y_data = color_ub_hum[mask_good_colors_ubvi_hum * mask_class_2_hum]
x_data_err = color_vi_err_hum[mask_good_colors_ubvi_hum * mask_class_2_hum]
y_data_err = color_ub_err_hum[mask_good_colors_ubvi_hum * mask_class_2_hum]
...
data_err = np.array([x_data_err, y_data_err]).T
...
def plot_gmm(gmm, X, label=True, ax=None):
    ...
    if label:
        ax.scatter(X[:, 0], X[:, 1], c=labels, s=40, cmap='viridis', zorder=0, alpha=0.1)
    else:
        ax.scatter(X[:, 0], X[:, 1], s=40, zorder=0, alpha=0.1)
    ax.axis('equal')
...
print(MoonsDataSet.shape)
MoonWeights = np.array([np.ones(100), np.ones(100)*10]).flatten()
print(MoonWeights.shape)
# plt.scatter(MoonsDataSet[:, 0], MoonsDataSet[:, 1])
# plt.show()
# exit()
