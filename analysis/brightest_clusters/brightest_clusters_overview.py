...
catalog_access = CatalogAccess(hst_cc_data_path=cluster_catalog_data_path,
...
for target in galaxy_name_list:
    ...
    mask_bright_clusters = (mass_ml > 1e6) & (target_name_ml == target)
    alma_data, alma_borders, rgb_img_hst_img, gc_img, cascade_img, young_img, rgb_img, wcs_hist = (
            get_image_data(galaxy_name=target, n_bins=n_bins, add_side_pixel_frac=add_side_pixel_frac,
                           kernel_size_prop=kernel_size_prop))

    figure = plt.figure(figsize=(30, 15))

    fontsize = 50
    ax_img = figure.add_axes([0.05, 0.05, 0.4, 0.9], projection=wcs_hist)
    ax_img_ycl = figure.add_axes([0.5, 0.05, 0.4, 0.9], projection=wcs_hist)


    ax_img.imshow(rgb_img_hst_img)

    ax_img_ycl.imshow(young_img)

    levels = [np.nanpercentile(alma_data, 90),
              np.nanpercentile(alma_data, 95),
              np.nanpercentile(alma_data, 99)]
    ax_img_ycl.contour(alma_data, levels=levels, colors='magenta', alpha=0.7, linewidths=4)
    ax_img_ycl.contour(alma_borders, levels=0, colors='k', linewidths=4)

    # plot bright clusters
    # for index in range()
    for ra, dec in zip(ra_ml[mask_bright_clusters], dec_ml[mask_bright_clusters]):
        coords_world_ml = SkyCoord(ra=ra*u.deg, dec=dec*u.deg)
        coords_pix_ml = wcs_hist.world_to_pixel(coords_world_ml)
        ax_img.scatter(coords_pix_ml[0],
                       coords_pix_ml[1],
                       color='white', marker='o', s=280, linewidth=4, facecolor='None')

        ax_img_ycl.scatter(coords_pix_ml[0],
                       coords_pix_ml[1],
                       color='red', marker='o', s=280, linewidth=4, facecolor='None')



    if not os.path.isdir('plot_output/%s' % target):
        os.makedirs('plot_output/%s' % target)
    figure.savefig('plot_output/%s/bright_cluster_distr_%s.png' % (target, target))
    plt.clf()
    plt.close("all")
