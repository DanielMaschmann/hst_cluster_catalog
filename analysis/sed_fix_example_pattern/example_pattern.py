...
def display_models(ax, y_color='nuvb', age_cut_sol50=5e2, age_dots_sol=None, age_dots_sol50=None, age_labels=False,
                   age_label_color='red', age_label_fontsize=30,
...
ax_ubvi = figure.add_axes([0.06, 0.705, 0.44, 0.29])
ax_bvvi = figure.add_axes([0.55, 0.705, 0.44, 0.29])

ax_pixel = figure.add_axes([0.04, 0.395, 0.44, 0.29])
ax_pixel_co = figure.add_axes([0.485, 0.395, 0.44, 0.29])
ax_co_cbar = figure.add_axes([0.93, 0.4375, 0.015, 0.21])


ax_ebv_age_old = figure.add_axes([0.06, 0.2, 0.44, 0.17])
ax_ebv_age_fix = figure.add_axes([0.55, 0.2, 0.44, 0.17])

ax_mass_age_old = figure.add_axes([0.06, 0.02, 0.44, 0.17])
ax_mass_age_fix = figure.add_axes([0.55, 0.02, 0.44, 0.17])

ax_ubvi.scatter(color_vi_ml_12, color_ub_ml_12, s=100, color='grey')
ax_bvvi.scatter(color_vi_ml_12, color_bv_ml_12, s=100, color='grey')

ax_ubvi.scatter(color_vi_ml_12[yro_fix_ml_12], color_ub_ml_12[yro_fix_ml_12], s=180, color='blue', label='YRO fix')
ax_ubvi.scatter(color_vi_ml_12[ogc_fix_ml_12], color_ub_ml_12[ogc_fix_ml_12], s=180, color='red', label='OGC fix')
ax_bvvi.scatter(color_vi_ml_12[yro_fix_ml_12], color_bv_ml_12[yro_fix_ml_12], s=180, color='blue')
ax_bvvi.scatter(color_vi_ml_12[ogc_fix_ml_12], color_bv_ml_12[ogc_fix_ml_12], s=180, color='red')
...
ax_ubvi.plot(vi_hull_gc_ubvi_ml_1_thresh_10, ub_hull_gc_ubvi_ml_1_thresh_10, color='darkred', linewidth=4)
ax_bvvi.plot(vi_hull_gc_bvvi_ml_1_thresh_10, bv_hull_gc_bvvi_ml_1_thresh_10, color='darkred', linewidth=4)

ax_ubvi.set_xlabel('V (F555W) - I (F814W)', fontsize=fontsize)
ax_bvvi.set_xlabel('V (F555W) - I (F814W)', fontsize=fontsize)
ax_ubvi.set_ylabel('U (F336W) - B (F438W)', labelpad=27, fontsize=fontsize)
ax_bvvi.set_ylabel('B (F438W) - V (F555W)', fontsize=fontsize)

ax_ubvi.set_xlim(x_lim_vi)
ax_bvvi.set_xlim(x_lim_vi)
ax_ubvi.set_ylim(y_lim_ub)
ax_bvvi.set_ylim(y_lim_bv)
ax_ubvi.tick_params(axis='both', which='both', width=1.5, length=4, right=True, top=True, direction='in', labelsize=fontsize)
ax_bvvi.tick_params(axis='both', which='both', width=1.5, length=4, right=True, top=True, direction='in', labelsize=fontsize)


ax_pixel.scatter(x_ml_12, y_ml_12, s=100, color='grey')
ax_pixel.scatter(x_ml_12[yro_fix_ml_12], y_ml_12[yro_fix_ml_12], s=180, color='blue')
ax_pixel.scatter(x_ml_12[ogc_fix_ml_12], y_ml_12[ogc_fix_ml_12], s=180, color='red')
...
ax_pixel_co.scatter(x_ml_12, y_ml_12, s=100, color='grey')

co_intensity[co_intensity == 0] = 0.05
ax_pixel_co.scatter(x_ml_12, y_ml_12, c=co_intensity, norm=norm_co, cmap=cmap_co, s=180)
...
ax_pixel.set_yticklabels(ax_pixel.get_yticklabels(), rotation=90, ha='right')
ax_pixel_co.set_yticklabels([])

ax_pixel.tick_params(axis='both', which='both', width=1.5, length=4, right=True, top=True, direction='in', labelsize=fontsize)
ax_pixel_co.tick_params(axis='both', which='both', width=1.5, length=4, right=True, top=True, direction='in', labelsize=fontsize)


random_age = np.random.uniform(low=-0.1, high=0.1, size=len(age_ml_12))
ax_ebv_age_old.scatter(np.log10(age_ir4_ml_12) + random_age, ebv_ir4_ml_12, s=100, color='grey')
ax_ebv_age_fix.scatter(np.log10(age_ml_12) + random_age, ebv_ml_12, s=100, color='grey')

ax_ebv_age_old.scatter(np.log10(age_ir4_ml_12[yro_fix_ml_12]) + random_age[yro_fix_ml_12], ebv_ir4_ml_12[yro_fix_ml_12], s=180, color='blue')
ax_ebv_age_old.scatter(np.log10(age_ir4_ml_12[ogc_fix_ml_12]) + random_age[ogc_fix_ml_12], ebv_ir4_ml_12[ogc_fix_ml_12], s=180, color='red')
ax_ebv_age_fix.scatter(np.log10(age_ml_12[yro_fix_ml_12]) + random_age[yro_fix_ml_12], ebv_ml_12[yro_fix_ml_12], s=180, color='blue')
ax_ebv_age_fix.scatter(np.log10(age_ml_12[ogc_fix_ml_12]) + random_age[ogc_fix_ml_12], ebv_ml_12[ogc_fix_ml_12], s=180, color='red')
...
ax_ebv_age_old.set_ylabel('Standard E(B-V) [mag]', labelpad=0, fontsize=fontsize)
ax_ebv_age_fix.set_ylabel('SED fix E(B-V) [mag]', labelpad=0, fontsize=fontsize)

ax_ebv_age_old.tick_params(axis='both', which='both', width=1.5, length=4, right=True, top=True, direction='in', labelsize=fontsize)
ax_ebv_age_fix.tick_params(axis='both', which='both', width=1.5, length=4, right=True, top=True, direction='in', labelsize=fontsize)


ax_mass_age_old.scatter(np.log10(age_ir4_ml_12) + random_age, mass_ir4_ml_12, s=100, color='grey')
ax_mass_age_fix.scatter(np.log10(age_ml_12) + random_age, mass_ml_12, s=100, color='grey')

ax_mass_age_old.scatter(np.log10(age_ir4_ml_12[yro_fix_ml_12]) + random_age[yro_fix_ml_12], mass_ir4_ml_12[yro_fix_ml_12], s=180, color='blue')
ax_mass_age_old.scatter(np.log10(age_ir4_ml_12[ogc_fix_ml_12]) + random_age[ogc_fix_ml_12], mass_ir4_ml_12[ogc_fix_ml_12], s=180, color='red')
ax_mass_age_fix.scatter(np.log10(age_ml_12[yro_fix_ml_12]) + random_age[yro_fix_ml_12], mass_ml_12[yro_fix_ml_12], s=180, color='blue')
ax_mass_age_fix.scatter(np.log10(age_ml_12[ogc_fix_ml_12]) + random_age[ogc_fix_ml_12], mass_ml_12[ogc_fix_ml_12], s=180, color='red')

ax_mass_age_old.set_ylabel(r'Standard M$_*$ [M$_{\odot}$]', labelpad=0, fontsize=fontsize)
ax_mass_age_fix.set_ylabel(r'SED fix M$_*$ [M$_{\odot}$]', labelpad=0, fontsize=fontsize)

ax_mass_age_old.set_xlabel('Standard log(age/yr)', labelpad=0, fontsize=fontsize)
ax_mass_age_fix.set_xlabel('SED fix log(age/yr)', labelpad=0, fontsize=fontsize)

ax_mass_age_old.set_yscale('log')
ax_mass_age_fix.set_yscale('log')

ax_mass_age_old.tick_params(axis='both', which='both', width=1.5, length=4, right=True, top=True, direction='in', labelsize=fontsize)
ax_mass_age_fix.tick_params(axis='both', which='both', width=1.5, length=4, right=True, top=True, direction='in', labelsize=fontsize)
