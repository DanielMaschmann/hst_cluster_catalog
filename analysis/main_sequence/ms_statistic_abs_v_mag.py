...
median_abs_v_mag_list_hum = []
frac_gc_list_hum = []
frac_cascade_list_hum = []
frac_young_list_hum = []
frac_no_class_list_hum = []
median_abs_v_mag_list_ml = []
frac_gc_list_ml = []
...
for index in range(len(target_list)):
    ...
    v = catalog_access.get_target_rotcur_v0(target=target_str) * u.km / u.s
    r = catalog_access.get_target_size_r90(target=target_str) * u.kpc

    dyn_time_scale_list.append((r.to(u.km)/v).to(u.yr).value)
    ...
    median_abs_v_mag_list_hum.append(np.nanmedian(abs_v_mag_hum_12))
    ...
    median_abs_v_mag_list_ml.append(np.nanmedian(abs_v_mag_ml_12))
...
print('dyn_time_scale_list ', dyn_time_scale_list)
print('dyn_time_scale_list ', np.nanmean(dyn_time_scale_list))
print('dyn_time_scale_list ', np.nanmin(dyn_time_scale_list))
print('dyn_time_scale_list ', np.nanmax(dyn_time_scale_list))
print(np.array(target_list)[dyn_time_scale_list == np.nanmin(dyn_time_scale_list)])

# get pearson coeeficients
pearson_gc_hum = stats.pearsonr(median_abs_v_mag_list_hum, frac_gc_list_hum).statistic
pearson_cascade_hum = stats.pearsonr(median_abs_v_mag_list_hum, frac_cascade_list_hum).statistic
pearson_young_hum = stats.pearsonr(median_abs_v_mag_list_hum, frac_young_list_hum).statistic
pearson_no_class_hum = stats.pearsonr(median_abs_v_mag_list_hum, frac_no_class_list_hum).statistic

pearson_gc_ml = stats.pearsonr(median_abs_v_mag_list_ml, frac_gc_list_ml).statistic
pearson_cascade_ml = stats.pearsonr(median_abs_v_mag_list_ml, frac_cascade_list_ml).statistic
pearson_young_ml = stats.pearsonr(median_abs_v_mag_list_ml, frac_young_list_ml).statistic
pearson_no_class_ml = stats.pearsonr(median_abs_v_mag_list_ml, frac_no_class_list_ml).statistic
...
median_abs_v_mag_list_hum = np.array(median_abs_v_mag_list_hum)
...
median_abs_v_mag_list_ml = np.array(median_abs_v_mag_list_ml)
...
mask_close = dist_list < 15

fig, ax = plt.subplots(ncols=4, nrows=2, sharex='all', sharey='row', figsize=(30, 17))
fontsize = 30

ax[0, 0].scatter(median_abs_v_mag_list_hum[mask_close], frac_young_list_hum[mask_close], s=200, color='tab:blue')
ax[0, 1].scatter(median_abs_v_mag_list_hum[mask_close], frac_cascade_list_hum[mask_close], s=200, color='tab:green')
ax[0, 2].scatter(median_abs_v_mag_list_hum[mask_close], frac_gc_list_hum[mask_close], s=200, color='tab:red')
ax[0, 3].scatter(median_abs_v_mag_list_hum[mask_close], frac_no_class_list_hum[mask_close], s=200, color='tab:gray')

ax[0, 0].scatter(median_abs_v_mag_list_ml[np.invert(mask_close)], frac_young_list_hum[np.invert(mask_close)], s=200, linewidth=4, facecolor='None', color='tab:blue')
ax[0, 1].scatter(median_abs_v_mag_list_ml[np.invert(mask_close)], frac_cascade_list_hum[np.invert(mask_close)], s=200, linewidth=4, facecolor='None', color='tab:green')
ax[0, 2].scatter(median_abs_v_mag_list_ml[np.invert(mask_close)], frac_gc_list_hum[np.invert(mask_close)], s=200, linewidth=4, facecolor='None', color='tab:red')
ax[0, 3].scatter(median_abs_v_mag_list_ml[np.invert(mask_close)], frac_no_class_list_hum[np.invert(mask_close)], s=200, linewidth=4, facecolor='None', color='tab:gray')



ax[1, 0].scatter(median_abs_v_mag_list_hum[mask_close], frac_young_list_ml[mask_close], s=200, color='tab:blue')
ax[1, 1].scatter(median_abs_v_mag_list_hum[mask_close], frac_cascade_list_ml[mask_close], s=200, color='tab:green')
ax[1, 2].scatter(median_abs_v_mag_list_hum[mask_close], frac_gc_list_ml[mask_close], s=200, color='tab:red')
ax[1, 3].scatter(median_abs_v_mag_list_hum[mask_close], frac_no_class_list_ml[mask_close], s=200, color='tab:gray')

ax[1, 0].scatter(median_abs_v_mag_list_ml[np.invert(mask_close)], frac_young_list_ml[np.invert(mask_close)], s=200, linewidth=4, facecolor='None', color='tab:blue')
ax[1, 1].scatter(median_abs_v_mag_list_ml[np.invert(mask_close)], frac_cascade_list_ml[np.invert(mask_close)], s=200, linewidth=4, facecolor='None', color='tab:green')
ax[1, 2].scatter(median_abs_v_mag_list_ml[np.invert(mask_close)], frac_gc_list_ml[np.invert(mask_close)], s=200, linewidth=4, facecolor='None', color='tab:red')
ax[1, 3].scatter(median_abs_v_mag_list_ml[np.invert(mask_close)], frac_no_class_list_ml[np.invert(mask_close)], s=200, linewidth=4, facecolor='None', color='tab:gray')

# linear fit
dummy_x_hum = np.linspace(np.min(median_abs_v_mag_list_hum), np.max(median_abs_v_mag_list_hum), 10)
dummy_x_ml = np.linspace(np.min(median_abs_v_mag_list_ml), np.max(median_abs_v_mag_list_ml), 10)

coef_cascade_hum, cov_cascade_hum = np.polyfit(median_abs_v_mag_list_hum, frac_cascade_list_hum,1, cov=True)
coef_cascade_ml, cov_cascade_ml = np.polyfit(median_abs_v_mag_list_ml, frac_cascade_list_ml,1, cov=True)
...
ax[0, 1].plot(dummy_x_hum, poly1d_fn_cascade_hum(dummy_x_hum), color='k', linestyle='--', linewidth=3,
              label=r'%.2f$(\pm %.2f)\times \Delta$MS + %.2f($\pm$%.2f)' %
                    (coef_cascade_hum[0], uncert_cascade_hum[0], coef_cascade_hum[1], uncert_cascade_hum[1]))
ax[1, 1].plot(dummy_x_ml, poly1d_fn_cascade_ml(dummy_x_ml), color='k', linestyle='--', linewidth=3,
              label=r'%.2f$(\pm %.2f)\times \Delta$MS + %.2f($\pm$%.2f)' %
...
ax[1, 0].set_xlabel(r'M$_{\rm V}$ [mag]', fontsize=fontsize)
ax[1, 1].set_xlabel(r'M$_{\rm V}$ [mag]', fontsize=fontsize)
ax[1, 2].set_xlabel(r'M$_{\rm V}$ [mag]', fontsize=fontsize)
ax[1, 3].set_xlabel(r'M$_{\rm V}$ [mag]', fontsize=fontsize)
...
# plt.tight_layout()
fig.savefig('plot_output/ms_stats_abs_v_band_mag.png')
