...
hist, x_bin, y_bin = np.histogram2d(log_stellar_mass[good_values], log_sfr[good_values],
                                    bins=(np.linspace(x_lim_low, x_lim_high, 70), np.linspace(y_lim_low, y_lim_high, 70)))

kernel = make_2dgaussian_kernel(4.0, size=9)  # FWHM = 3.0
hist = convolve(hist, kernel)

# hist[hist < 10] = np.nan
cmap_ms = matplotlib.cm.get_cmap('Greys')
norm_ms = matplotlib.colors.Normalize(vmin=0, vmax=np.nanmax(hist))
...
for index in range(len(target_list)):
    ...
    contours(ax=ax1, x=color_vi_ml_12[good_colors * detect_nuvu_ml_12 * detect_nuvb_ml_12],
             y=color_ub_ml_12[good_colors * detect_nuvu_ml_12 * detect_nuvb_ml_12], levels=[0.3, 0.5, 0.7, 0.8, 0.95, 0.99])
