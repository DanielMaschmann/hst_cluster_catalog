...
for index in range(0, 20):
    ...
    age_err_12_hum = catalog_access.get_hst_cc_age_err(target=target)
    m_star_12_hum = catalog_access.get_hst_cc_stellar_m(target=target)
    m_star_err_12_hum = catalog_access.get_hst_cc_stellar_m_err(target=target)
    ...
    age_err_3_hum = catalog_access.get_hst_cc_age_err(target=target, cluster_class='class3')
    m_star_3_hum = catalog_access.get_hst_cc_stellar_m(target=target, cluster_class='class3')
    m_star_err_3_hum = catalog_access.get_hst_cc_stellar_m_err(target=target, cluster_class='class3')
    ...
    age_err_12_ml = catalog_access.get_hst_cc_age_err(target=target, classify='ml')
    m_star_12_ml = catalog_access.get_hst_cc_stellar_m(target=target, classify='ml')
    m_star_err_12_ml = catalog_access.get_hst_cc_stellar_m_err(target=target, classify='ml')
    ...
    age_err_3_ml = catalog_access.get_hst_cc_age_err(target=target, classify='ml', cluster_class='class3')
    m_star_3_ml = catalog_access.get_hst_cc_stellar_m(target=target, classify='ml', cluster_class='class3')
    m_star_err_3_ml = catalog_access.get_hst_cc_stellar_m_err(target=target, classify='ml', cluster_class='class3')
    ...
    age_hum = age_12_hum #np.concatenate([age_12_hum, age_3_hum])
    age_err_hum = age_err_12_hum #np.concatenate([age_err_12_hum, age_err_3_hum])
    m_star_hum = m_star_12_hum #np.concatenate([m_star_12_hum, m_star_3_hum])
    m_star_err_hum = m_star_err_12_hum #np.concatenate([m_star_err_12_hum, m_star_err_3_hum])

    age_ml = age_12_ml #np.concatenate([age_12_ml, age_3_ml])
    age_err_ml = age_err_12_ml #np.concatenate([age_err_12_ml, age_err_3_ml])
    m_star_ml = m_star_12_ml #np.concatenate([m_star_12_ml, m_star_3_ml])
    m_star_err_ml = m_star_err_12_ml #np.concatenate([m_star_err_12_ml, m_star_err_3_ml])
    ...
    age_log_err_hum = np.zeros(len(age_err_hum))
    age_log_err_upper_hum = np.log10(age_hum + age_err_hum) - np.log10(age_hum)
    age_log_err_lower_hum = np.log10(age_hum - age_err_hum) - np.log10(age_hum)
    mask_bad_lower_error = np.isnan(age_log_err_lower_hum)
    age_log_err_hum[np.invert(mask_bad_lower_error)] = (
            (age_log_err_upper_hum + np.abs(age_log_err_lower_hum))[np.invert(mask_bad_lower_error)] / 2)
    age_log_err_hum[mask_bad_lower_error] = age_log_err_upper_hum[mask_bad_lower_error]
    age_hum = np.log10(age_hum) + 6

    m_star_log_err_hum = np.zeros(len(m_star_err_hum))
    m_star_log_err_upper_hum = np.log10(m_star_hum + m_star_err_hum) - np.log10(m_star_hum)
    m_star_log_err_lower_hum = np.log10(m_star_hum - m_star_err_hum) - np.log10(m_star_hum)
    mask_bad_lower_error = np.isnan(m_star_log_err_lower_hum)
    m_star_log_err_hum[np.invert(mask_bad_lower_error)] = (
            (m_star_log_err_upper_hum + np.abs(m_star_log_err_lower_hum))[np.invert(mask_bad_lower_error)] / 2)
    m_star_log_err_hum[mask_bad_lower_error] = m_star_log_err_upper_hum[mask_bad_lower_error]


    age_log_err_ml = np.zeros(len(age_err_ml))
    age_log_err_upper_ml = np.log10(age_ml + age_err_ml) - np.log10(age_ml)
    age_log_err_lower_ml = np.log10(age_ml - age_err_ml) - np.log10(age_ml)
    mask_bad_lower_error = np.isnan(age_log_err_lower_ml)
    ...
    age_ml = np.log10(age_ml) + 6

    m_star_log_err_ml = np.zeros(len(m_star_err_ml))
    m_star_log_err_upper_ml = np.log10(m_star_ml + m_star_err_ml) - np.log10(m_star_ml)
    m_star_log_err_lower_ml = np.log10(m_star_ml - m_star_err_ml) - np.log10(m_star_ml)
    mask_bad_lower_error = np.isnan(m_star_log_err_lower_ml)
    ...
    gauss_map_hum = np.zeros((len(x_bins_gauss), len(y_bins_gauss)))
    gauss_map_ml = np.zeros((len(x_bins_gauss), len(y_bins_gauss)))

    mask_bad_values_hum = (np.isinf(age_log_err_hum)) | (np.isinf(m_star_log_err_hum)) | (age_err_hum == 0) | (m_star_err_hum == 0)
    mask_bad_values_ml = (np.isinf(age_log_err_ml)) | (np.isinf(m_star_log_err_ml)) | (age_err_ml == 0) | (m_star_err_ml == 0)

    for color_index in range(len(age_hum)):
        if mask_bad_values_hum[color_index]:
            continue
        x_err = np.sqrt(age_log_err_hum[color_index]**2 + 0.05**2)
        y_err = np.sqrt(m_star_log_err_hum[color_index]**2 + 0.05**2)
        gauss = helper_func.gauss2d(x=x_mesh, y=y_mesh, x0=age_hum[color_index], y0=np.log10(m_star_hum)[color_index],
                        sig_x=x_err, sig_y=y_err)
        gauss_map_hum += gauss

    for color_index in range(len(age_ml)):
        if mask_bad_values_ml[color_index]:
            continue
        x_err = np.sqrt(age_log_err_ml[color_index]**2 + 0.05**2)
        y_err = np.sqrt(m_star_log_err_ml[color_index]**2 + 0.05**2)
        gauss = helper_func.gauss2d(x=x_mesh, y=y_mesh, x0=age_ml[color_index], y0=np.log10(m_star_ml)[color_index],
                        sig_x=x_err, sig_y=y_err)
        gauss_map_ml += gauss


    ax_hum[row_index, col_index].imshow(gauss_map_hum, origin='lower', cmap='Greys',
                                        extent=(age_lim[0], age_lim[1], mass_lim[0], mass_lim[1]),
                                        interpolation='nearest', aspect='auto')
    ax_ml[row_index, col_index].imshow(gauss_map_ml, origin='lower', cmap='Greys',
                                        extent=(age_lim[0], age_lim[1], mass_lim[0], mass_lim[1]),
                                       interpolation='nearest', aspect='auto')
...
fig_hum.savefig('plot_output/age_m_star_hum_1_dens_c12.png', bbox_inches='tight', dpi=300)
# fig_hum.savefig('plot_output/age_m_star_hum_1.pdf', bbox_inches='tight', dpi=300)

fig_ml.subplots_adjust(wspace=0, hspace=0)
fig_ml.savefig('plot_output/age_m_star_ml_1_dens_c12.png', bbox_inches='tight', dpi=300)
