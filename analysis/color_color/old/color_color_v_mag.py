...
for index in range(0, len(target_list)):
    ...
    v_mag_hum = np.concatenate([v_mag_hum, v_mag_hum_12, v_mag_hum_3])
    ...
    v_mag_ml = np.concatenate([v_mag_ml, v_mag_ml_12, v_mag_ml_3])
...
v_mag_hist_cl1_hum = np.zeros((29, 29)) * np.nan
v_mag_hist_cl2_hum = np.zeros((29, 29)) * np.nan
v_mag_hist_cl12_hum = np.zeros((29, 29)) * np.nan
v_mag_hist_cl3_hum = np.zeros((29, 29)) * np.nan


v_mag_hist_cl1_ml = np.zeros((29, 29)) * np.nan
v_mag_hist_cl2_ml = np.zeros((29, 29)) * np.nan
v_mag_hist_cl12_ml = np.zeros((29, 29)) * np.nan
v_mag_hist_cl3_ml = np.zeros((29, 29)) * np.nan
...
for x_index in range(len(x_bins) - 1):
    for y_index in range(len(y_bins) - 1):
        ...
        if sum(mask_1_hum) > threshold_hum:
            v_mag_hist_cl1_hum[x_index, y_index] = np.nanmedian(v_mag_hum[good_data_hum*mask_class_1_hum][mask_1_hum])
        if sum(mask_2_hum) > threshold_hum:
            v_mag_hist_cl2_hum[x_index, y_index] = np.nanmedian(v_mag_hum[good_data_hum*mask_class_2_hum][mask_2_hum])
        if sum(mask_12_hum) > threshold_hum:
            v_mag_hist_cl12_hum[x_index, y_index] = np.nanmedian(
                v_mag_hum[good_data_hum*(mask_class_2_hum + mask_class_1_hum)][mask_12_hum])
        if sum(mask_3_hum) > threshold_hum:
            v_mag_hist_cl3_hum[x_index, y_index] = np.nanmedian(v_mag_hum[good_data_hum*mask_class_3_hum][mask_3_hum])

        if sum(mask_1_ml) > threshold_ml:
            v_mag_hist_cl1_ml[x_index, y_index] = np.nanmedian(v_mag_ml[good_data_ml*mask_class_1_ml][mask_1_ml])
        if sum(mask_2_ml) > threshold_ml:
            v_mag_hist_cl2_ml[x_index, y_index] = np.nanmedian(v_mag_ml[good_data_ml*mask_class_2_ml][mask_2_ml])
        if sum(mask_12_ml) > threshold_ml:
            v_mag_hist_cl12_ml[x_index, y_index] = np.nanmedian(
                v_mag_ml[good_data_ml*(mask_class_2_ml + mask_class_1_ml)][mask_12_ml])
        if sum(mask_3_ml) > threshold_ml:
            v_mag_hist_cl3_ml[x_index, y_index] = np.nanmedian(v_mag_ml[good_data_ml*mask_class_3_ml][mask_3_ml])
...
norm_v_mag = Normalize(20, 25)
cmap_v_mag = 'viridis'

ax_1_hum.imshow(v_mag_hist_cl1_hum.T, extent=(-1.0, 2.3, 1.25, -2.2), cmap=cmap_v_mag, norm=norm_v_mag)
ax_2_hum.imshow(v_mag_hist_cl2_hum.T, extent=(-1.0, 2.3, 1.25, -2.2), cmap=cmap_v_mag, norm=norm_v_mag)
ax_12_hum.imshow(v_mag_hist_cl12_hum.T, extent=(-1.0, 2.3, 1.25, -2.2), cmap=cmap_v_mag, norm=norm_v_mag)
ax_3_hum.imshow(v_mag_hist_cl3_hum.T, extent=(-1.0, 2.3, 1.25, -2.2), cmap=cmap_v_mag, norm=norm_v_mag)

ax_1_ml.imshow(v_mag_hist_cl1_ml.T, extent=(-1.0, 2.3, 1.25, -2.2), cmap=cmap_v_mag, norm=norm_v_mag)
ax_2_ml.imshow(v_mag_hist_cl2_ml.T, extent=(-1.0, 2.3, 1.25, -2.2), cmap=cmap_v_mag, norm=norm_v_mag)
ax_12_ml.imshow(v_mag_hist_cl12_ml.T, extent=(-1.0, 2.3, 1.25, -2.2), cmap=cmap_v_mag, norm=norm_v_mag)
ax_3_ml.imshow(v_mag_hist_cl3_ml.T, extent=(-1.0, 2.3, 1.25, -2.2), cmap=cmap_v_mag, norm=norm_v_mag)


ColorbarBase(ax_cbar, orientation='vertical', cmap=cmap_v_mag, norm=norm_v_mag, extend='neither', ticks=None)
ax_cbar.set_ylabel(r'V-mag', labelpad=0, fontsize=fontsize)
...
fig.savefig('plot_output/ub_vi_v_mag.png')
fig.savefig('plot_output/ub_vi_v_mag.pdf')
fig.clf()
