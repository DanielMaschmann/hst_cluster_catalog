...
chi2_hist_cl1_hum = np.zeros((29, 29)) * np.nan
chi2_hist_cl2_hum = np.zeros((29, 29)) * np.nan
chi2_hist_cl12_hum = np.zeros((29, 29)) * np.nan
chi2_hist_cl3_hum = np.zeros((29, 29)) * np.nan


chi2_hist_cl1_ml = np.zeros((29, 29)) * np.nan
chi2_hist_cl2_ml = np.zeros((29, 29)) * np.nan
chi2_hist_cl12_ml = np.zeros((29, 29)) * np.nan
chi2_hist_cl3_ml = np.zeros((29, 29)) * np.nan
...
for x_index in range(len(x_bins) - 1):
    for y_index in range(len(y_bins) - 1):
        ...
        if sum(mask_1_hum) > threshold_hum:
            chi2_hist_cl1_hum[x_index, y_index] = np.nanmean(chi2_hum[good_data_hum*mask_class_1_hum][mask_1_hum])
        if sum(mask_2_hum) > threshold_hum:
            chi2_hist_cl2_hum[x_index, y_index] = np.nanmean(chi2_hum[good_data_hum*mask_class_2_hum][mask_2_hum])
        if sum(mask_12_hum) > threshold_hum:
            chi2_hist_cl12_hum[x_index, y_index] = np.nanmean(
                chi2_hum[good_data_hum*(mask_class_2_hum + mask_class_1_hum)][mask_12_hum])
        if sum(mask_3_hum) > threshold_hum:
            chi2_hist_cl3_hum[x_index, y_index] = np.nanmean(chi2_hum[good_data_hum*mask_class_3_hum][mask_3_hum])

        if sum(mask_1_ml) > threshold_ml:
            chi2_hist_cl1_ml[x_index, y_index] = np.nanmean(chi2_ml[good_data_ml*mask_class_1_ml][mask_1_ml])
        if sum(mask_2_ml) > threshold_ml:
            chi2_hist_cl2_ml[x_index, y_index] = np.nanmean(chi2_ml[good_data_ml*mask_class_2_ml][mask_2_ml])
        if sum(mask_12_ml) > threshold_ml:
            chi2_hist_cl12_ml[x_index, y_index] = np.nanmean(
                chi2_ml[good_data_ml*(mask_class_2_ml + mask_class_1_ml)][mask_12_ml])
        if sum(mask_3_ml) > threshold_ml:
            chi2_hist_cl3_ml[x_index, y_index] = np.nanmean(chi2_ml[good_data_ml*mask_class_3_ml][mask_3_ml])
...
ax_cbar = fig.add_axes([0.955, 0.26, 0.02, 0.5])
...
norm_chi2 = Normalize(0, 5)
cmap_chi2 = 'viridis'

ax_1_hum.imshow(chi2_hist_cl1_hum.T, extent=(-1.0, 2.3, 1.25, -2.2), cmap=cmap_chi2, norm=norm_chi2)
ax_2_hum.imshow(chi2_hist_cl2_hum.T, extent=(-1.0, 2.3, 1.25, -2.2), cmap=cmap_chi2, norm=norm_chi2)
ax_12_hum.imshow(chi2_hist_cl12_hum.T, extent=(-1.0, 2.3, 1.25, -2.2), cmap=cmap_chi2, norm=norm_chi2)
ax_3_hum.imshow(chi2_hist_cl3_hum.T, extent=(-1.0, 2.3, 1.25, -2.2), cmap=cmap_chi2, norm=norm_chi2)

ax_1_ml.imshow(chi2_hist_cl1_ml.T, extent=(-1.0, 2.3, 1.25, -2.2), cmap=cmap_chi2, norm=norm_chi2)
ax_2_ml.imshow(chi2_hist_cl2_ml.T, extent=(-1.0, 2.3, 1.25, -2.2), cmap=cmap_chi2, norm=norm_chi2)
ax_12_ml.imshow(chi2_hist_cl12_ml.T, extent=(-1.0, 2.3, 1.25, -2.2), cmap=cmap_chi2, norm=norm_chi2)
ax_3_ml.imshow(chi2_hist_cl3_ml.T, extent=(-1.0, 2.3, 1.25, -2.2), cmap=cmap_chi2, norm=norm_chi2)


ColorbarBase(ax_cbar, orientation='vertical', cmap=cmap_chi2, norm=norm_chi2, extend='neither', ticks=[1, 2, 3, 4])
ax_cbar.set_ylabel(r'$\chi^{2}$', labelpad=0, fontsize=fontsize)
...
fig.savefig('plot_output/ub_vi_chi2.png')
fig.savefig('plot_output/ub_vi_chi2.pdf')
fig.clf()
