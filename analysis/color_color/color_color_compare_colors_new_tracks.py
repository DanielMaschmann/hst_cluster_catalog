...
bv_annotation_dict = {
    5: {'offset': [-0.5, +0.3], 'label': '5 Myr', 'ha': 'right', 'va': 'center'},
    10: {'offset': [+0.7, +0.3], 'label': '10 Myr', 'ha': 'left', 'va': 'center'},
...
def display_models(ax, y_color='nuvb',
                   ...
                   color_cb19_sol='goldenrod', linewidth_cb19_sol=4, linestyle_cb19_sol='-',
                   label_sol=None, label_sol50=None, label_cb19_sol=None):

    y_model_sol = globals()['model_%s_sol' % y_color]
    y_model_sol50 = globals()['model_%s_sol50' % y_color]
    y_model_cb19_sol = globals()['model_%s_cb19_sol' % y_color]

    ax.plot(model_vi_sol, y_model_sol, color=color_sol, linewidth=linewidth_sol, linestyle=linestyle_sol, zorder=10,
            label=label_sol)
    ax.plot(model_vi_cb19_sol, y_model_cb19_sol, color=color_cb19_sol, linewidth=linewidth_cb19_sol, linestyle=linestyle_cb19_sol, zorder=10,
            label=label_cb19_sol)
    ...
    for age in age_dots_sol:
        ...
        ax.scatter(model_vi_cb19_sol[age_mod_cb19_sol == age], y_model_cb19_sol[age_mod_cb19_sol == age], color='olive', s=80, zorder=20)
    ...
    if age_labels:
        ...
        for age in annotation_dict.keys():
            ...
            txt_cb19_sol = ax.annotate(' ', #annotation_dict[age]['label'],
                        xy=(model_vi_cb19_sol[age_mod_cb19_sol == age], y_model_cb19_sol[age_mod_cb19_sol == age]),
                        xytext=(model_vi_sol[age_mod_sol == age]+annotation_dict[age]['offset'][0],
                                y_model_cb19_sol[age_mod_cb19_sol == age]+annotation_dict[age]['offset'][1]),
                        fontsize=age_label_fontsize, xycoords='data', textcoords='data', color=age_label_color,
                        ha=annotation_dict[age]['ha'], va=annotation_dict[age]['va'], zorder=30,
                              arrowprops=dict(arrowstyle='-|>', color='darkorange', lw=3, ls='-'),
                        path_effects=[patheffects.withStroke(linewidth=3,
                                                        foreground="w")])
            txt_cb19_sol.arrow_patch.set_path_effects([patheffects.Stroke(linewidth=5, foreground="w"),
                                                  patheffects.Normal()])
...
model_cb19_sol_nuv_flux = hf.conv_ab_mag2mjy(photo_values['UVIS1_f275w_AB'])
model_cb19_sol_u_flux = hf.conv_ab_mag2mjy(photo_values['UVIS1_f336w_AB'])
model_cb19_sol_b_flux = hf.conv_ab_mag2mjy(photo_values['UVIS1_f438w_AB'])
model_cb19_sol_v_flux = hf.conv_ab_mag2mjy(photo_values['UVIS1_f555w_AB'])
model_cb19_sol_i_flux = hf.conv_ab_mag2mjy(photo_values['UVIS1_f814w_AB'])

model_cb19_sol_nuv_vega = hf.conv_mjy2vega(flux=model_cb19_sol_nuv_flux, ab_zp=catalog_access.get_zp_mag(target='ngc7496', band='F275W', mag_sys='AB'), vega_zp=catalog_access.get_zp_mag(target='ngc7496', band='F275W'))
model_cb19_sol_u_vega = hf.conv_mjy2vega(flux=model_cb19_sol_u_flux, ab_zp=catalog_access.get_zp_mag(target='ngc7496', band='F336W', mag_sys='AB'), vega_zp=catalog_access.get_zp_mag(target='ngc7496', band='F336W'))
model_cb19_sol_b_vega = hf.conv_mjy2vega(flux=model_cb19_sol_b_flux, ab_zp=catalog_access.get_zp_mag(target='ngc7496', band='F438W', mag_sys='AB'), vega_zp=catalog_access.get_zp_mag(target='ngc7496', band='F438W'))
model_cb19_sol_v_vega = hf.conv_mjy2vega(flux=model_cb19_sol_v_flux, ab_zp=catalog_access.get_zp_mag(target='ngc7496', band='F555W', mag_sys='AB'), vega_zp=catalog_access.get_zp_mag(target='ngc7496', band='F555W'))
model_cb19_sol_i_vega = hf.conv_mjy2vega(flux=model_cb19_sol_i_flux, ab_zp=catalog_access.get_zp_mag(target='ngc7496', band='F814W', mag_sys='AB'), vega_zp=catalog_access.get_zp_mag(target='ngc7496', band='F814W'))
...
display_models(ax=ax[0, 0], age_label_fontsize=fontsize+2,
               label_sol=r'BC03, Z$_{\odot}$',
               label_sol50=r'BC03, Z$_{\odot}/50\,(> 500\,{\rm Myr})$',
               label_cb19_sol=r'CB19, Z$_{\odot}$',)
...
ax[0, 0].text(x_lim_vi[0] + (x_lim_vi[1]-x_lim_vi[0])*0.05, y_lim_nuvb[0] + (y_lim_nuvb[1]-y_lim_nuvb[0])*0.31,
...
# plt.tight_layout()
fig.savefig('plot_output/cc_compare_new_trac.png')
# fig.savefig('plot_output/cc_compare_new_trac.pdf')
fig.clf()
plt.close()
