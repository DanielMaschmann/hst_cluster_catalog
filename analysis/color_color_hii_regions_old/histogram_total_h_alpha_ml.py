...
for index in range(len(target_list)):
    ...
    hii_ml_12 = np.zeros(len(color_vi_ml_12))
    hii_ml_3 = np.zeros(len(color_vi_ml_3))
    if target == 'ngc0685':
        target_ext = 'ngc685'
    ...
    if os.path.isfile(extra_file):
        ...
        for running_index, cluster_id in enumerate(cluster_id_ml_12):
            ...
            hii_ml_12[running_index] = extra_hii_mask[index_extra_table]
        for running_index, cluster_id in enumerate(cluster_id_ml_3):
            ...
            hii_ml_3[running_index] = extra_hii_mask[index_extra_table]
    else:
        print(extra_file, ' does not exist')

    age_array = np.concatenate([age_array, age_ml_12, age_ml_3])
    n_h_alpha_array = np.concatenate([n_h_alpha_array, h_alpha_intensity_ml_12, h_alpha_intensity_ml_3])
    class_array = np.concatenate([class_array, cluster_class_ml_12, cluster_class_ml_3])
    hii_array = np.concatenate([hii_array, hii_ml_12, hii_ml_3])
...
ax[0, 0].set_title('Class 1 ML', fontsize=fontsize)
ax[0, 1].set_title('Class 2 ML', fontsize=fontsize)
ax[0, 2].set_title('Class 3 ML', fontsize=fontsize)
...
fig.savefig('plot_output/hist_h_alpha_total_ml.png', bbox_inches='tight', dpi=300)
