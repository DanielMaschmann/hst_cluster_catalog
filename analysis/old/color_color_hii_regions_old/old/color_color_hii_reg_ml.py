...
for index in range(0, 20):
    ...
    if os.path.isfile(hii_mask_file_name) & (target != 'ngc4254') & (target != 'ngc4298'):
        ...
        mask_hii_reg = np.zeros(len(color_vi_ml_12), dtype=bool)
    ...
    ax[row_index, col_index].scatter(color_vi_ml_12[(cluster_class_ml_12 == 1) & ~mask_hii_reg],
                                     color_ub_ml_12[(cluster_class_ml_12 == 1) & ~mask_hii_reg],
                                     c='silver', s=10)
    ax[row_index, col_index].scatter(color_vi_ml_12[(cluster_class_ml_12 == 2) & ~mask_hii_reg],
                                         color_ub_ml_12[(cluster_class_ml_12 == 2) & ~mask_hii_reg],
                                     c='silver', s=10)
...
fig.savefig('plot_output/ub_vi_hii_reg_ml_1.png', bbox_inches='tight', dpi=300)
fig.savefig('plot_output/ub_vi_hii_reg_ml_1.pdf', bbox_inches='tight', dpi=300)
...
for index in range(20, 39):
    ...
    if target == 'ngc0628c':
        ...
        target_hii = target
    hii_mask_file_name = '/home/benutzer/data/PHANGS_products/NB_HIIregions/v0/HII_masks/%s_NB_HII_mask.fits' % target_hii.upper()
    if os.path.isfile(hii_mask_file_name) & (target != 'ngc1087') & (target != 'ngc1512') & (target != 'ngc7496') & (target != 'ngc1300') & (target != 'ngc1317'):
        hii_reg_tab_hdu = fits.open(hii_mask_file_name)
        hii_reg_data_table = hii_reg_tab_hdu[0].data
...
fig.savefig('plot_output/ub_vi_hii_reg_ml_2.png', bbox_inches='tight', dpi=300)
fig.savefig('plot_output/ub_vi_hii_reg_ml_2.pdf', bbox_inches='tight', dpi=300)
