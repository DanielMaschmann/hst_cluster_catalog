...
max_m_satr = np.zeros(len(target_list))
...
for index, target in enumerate(target_list):
    ...
    if target == 'ngc0628':
        ...
        m_star = catalog_access.get_hst_cc_stellar_m(target='ngc0628c')
        age = catalog_access.get_hst_cc_age(target='ngc0628c')
        ...
        m_star = catalog_access.get_hst_cc_stellar_m(target=target)
    ...
    sfr_err[index] = catalog_access.get_target_sfr_err(target=target)
    ...
    max_m_satr[index] = np.nanmax(m_star[age < 100])
...
fig, ax = plt.subplots(nrows=1, sharex=True, figsize=(13, 11))
fontsize = 26
...
ax.scatter(sfr, max_m_satr, s=120, color='darkslategrey', label='Human-classified', zorder=10)
# ax[0].scatter(sfr, number_ml_12, s=120, color='yellowgreen', label='ML-classified', zorder=10)
...
ax.set_xlabel('SFR')
ax.set_ylabel('M_star most massive clauster < 100Myr')
...
for i in range(len(sfr)):
    ax[1].plot([sfr[i], sfr[i]], [number_ml_3[i], number_hum_3[i]], color='grey', linestyle='--', linewidth=3)
ax[1].scatter(sfr, number_hum_3, s=120, color='darkslategrey', zorder=10)
ax[1].scatter(sfr, number_ml_3, s=120, color='yellowgreen', zorder=10)
